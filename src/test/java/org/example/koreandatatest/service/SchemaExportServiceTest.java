...
class SchemaExportServiceTest {
  ...
  @Test
  void givenFileTypeAndSchemaAndRowCount_whenExporting_thenMarksEntityExport() {
    // Given
    ExportFileType exportFileType = ExportFileType.CSV;
    TableSchemaDto dto = TableSchemaDto.of("test_schema", "jang", null, null);
    int rowCount = 5;
  ...
  void givenNoUserIdInParams_whenExporting_thenDoesNotTrySelectingTableSchema() {
    ...
    ExportFileType exportFileType = ExportFileType.CSV;
    String userId = null;
    TableSchemaDto dto = TableSchemaDto.of("test_schema", userId, null, null);
    int rowCount = 5;
    given(mockDataFileExporterContext.export(exportFileType, dto,rowCount)).willReturn("test,file,format");
